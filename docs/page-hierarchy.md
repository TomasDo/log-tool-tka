...
股骨远端验证 / 股骨后方验证 / 胫骨近端验证 悬停：验证工具 `collect check` 对比该会话最近一次 `plan`；|Δ| >1 mm 或 1° 标注，>2 mm 或 2° 重点。缺 collect 的参数标「未采集」；只做了对应截骨未进验证时，未采集挂在截骨块上。不改 L1/L2/L3 归属。
...
手术轨道上的色块（同一 uuid 一台手术）同时驱动轨道窗口底部的 **方案汇总**：每张卡片对应一台已打开方案的手术，列出必要步骤是否完成；点卡片或某一步跳到该手术起始行（或该步证据行）。
