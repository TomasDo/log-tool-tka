...
- 每个截骨面的参数原则上都应采集：股骨远端（远端内/外侧截骨量、内外翻角、前倾角）、股骨后方（后方内/外侧截骨量、旋转角）、胫骨近端（近端内/外侧截骨量、后倾角、旋转角、内外翻角）。进入对应验证步（或只做了截骨未进验证）但缺 `collect check` 的，在该二级块上标注（key）「未采集」。未做到截骨也未进验证的会话不标。只截骨未验证时，未采集挂在截骨块上。
- 胫骨内外翻也要求采集。当前样本从未打出 `collect check tibia varus`，仍标「未采集」；方案有 `plan tibia varus`，出现 collect 后即可比较 Δ。
...
- 配准页 check = 检查配准精度；导航页 check = 实测截骨量 vs 方案目标；截骨量 |Δ|>1 mm 标注、>2 mm 重点；角度同样 1°/2°；每个截骨面参数都应采集，缺的标「未采集」
